...
-nnet josh
-xgboosttreemodel/gbm/adaboost yujing
-randomforest sangil
```{r}
...
# test <- bake(prep(rec), new_data = dat2)

# res_impute_knn <- resample(rec, model = RandomForestModel, control = ctrl)
## now working on model fit. 
mspec <- ModelSpecification(
   rec,
   model = grid.model,
   control = ctrl
) %>% set_optim_bfgs(progress = TRUE, verbose = 1)
mspec

rf_fit <- fit(mspec)
res_rf <- resample(mspec)

# mlfit <- fit(mspec)
(tuned_model <- as.MLModel(rf_fit))
summary(tuned_model)
performance(tuned_model)
